---
nome: Avenged Sevenfold
genero: Metal
origem: Huntington Beach, Califórnia, EUA
ano: 1999
fotoCapa: /images/avenged-sevenfold-band-sonar.jpg
fotoCapaAlt: Banda de metal se apresentando ao vivo no palco, sob luzes intensas
fotoCapaPosition: 40
blurb: Duas guitarras técnicas em disputa cordial, um vocal que evoluiu do grito
  hardcore pro registro melódico e uma reverência assumida ao heavy metal
  clássico de Sabbath, Maiden e Metallica.
integrantes:
  - nome: M. Shadows
    funcao: Vocal
    status: Membro fundador
    anos: 1999 – hoje
    inspiracoes: Guns N' Roses, Iron Maiden, Metallica
    resumo: M. Shadows (Matthew Charles Sanders) é vocalista e cofundador do
      Avenged Sevenfold, criado em Huntington Beach, Califórnia, em 1999. Nos
      dois primeiros discos da banda cantava majoritariamente em vocais
      guturais de matriz hardcore; a partir de "City of Evil" (2005) passou a
      treinar meses a fio com um vocal coach que já havia trabalhado com Axl
      Rose e Chris Cornell, em busca de um canto limpo, potente e melódico sem
      perder a agressividade. É também um dos principais letristas e
      compositores do grupo.
    pontos:
      - Transição de vocais guturais hardcore pra um registro limpo e
        melódico a partir de "City of Evil".
      - Um dos principais compositores e letristas da banda, ao lado de
        Zacky Vengeance.
    setup:
      - tipo: Microfone
        original:
          nome: Audio-Technica AE6100
          preco: ~R$ 2.500
        low:
          nome: Shure SM58
          preco: ~R$ 900
          obs: Dinâmico robusto e testado em palco por uma fração do preço.
      - tipo: Monitoração
        original:
          nome: In-ear Shure PSM 300
          preco: ~R$ 5.500
        low:
          nome: Behringer Powerplay In-ear
          preco: ~R$ 900
          obs: Retorno limpo pra acompanhar a troca de registro vocal.
  - nome: Synyster Gates
    funcao: Guitarra solo
    status: Guitarrista solo
    anos: 1999 – hoje
    inspiracoes: Eddie Van Halen, jazz (Musicians Institute), Brian Haner Sr.
    resumo: Synyster Gates (Brian Elwin Haner Jr.) entrou pro Avenged Sevenfold
      ainda no fim de 1999, pouco depois da formação da banda, e é o
      guitarrista solo do grupo desde então. Filho do músico Brian Haner Sr.,
      estudou teoria e improvisação de jazz no Musicians Institute, em Los
      Angeles, formação que ajuda a explicar frases melódicas e harmônicas
      incomuns pro universo do heavy metal. Tem guitarras e amplificadores
      signature pela Schecter há quase duas décadas.
    pontos:
      - Formação em jazz que traz frases fora do padrão do metal
        tradicional.
      - Solos rápidos com alternate picking, arpejos e escalas pouco comuns
        no gênero.
    setup:
      - tipo: Guitarra
        original:
          nome: Schecter Synyster Gates Custom-S
          preco: ~R$ 9.500
        low:
          nome: Schecter C-1 FR-S Apocalypse
          preco: ~R$ 4.200
          obs: Captação ativa e trastes extra jumbo parecidos por menos.
      - tipo: Amplificação
        original:
          nome: Schecter Hellwin Stage 100 (signature)
          preco: ~R$ 18.000
        low:
          nome: Boss Katana 100 MkII
          preco: ~R$ 2.400
          obs: Canais de alto ganho versáteis pros solos mais técnicos.
  - nome: Zacky Vengeance
    funcao: Guitarra base
    status: Membro fundador
    anos: 1999 – hoje
    inspiracoes: Iron Maiden, Guns N' Roses, Metallica
    resumo: Zacky Vengeance (Zachary James Baker) é guitarrista base e
      cofundador do Avenged Sevenfold, tendo criado a banda ao lado de M.
      Shadows em 1999, ainda no ensino médio, em Huntington Beach. Autodidata,
      é responsável pela camada de riffs pesados que sustenta os solos mais
      técnicos de Synyster Gates, além de assinar parte relevante das
      composições da banda desde os primeiros discos.
    pontos:
      - Riffs pesados e cortantes que sustentam a base rítmica ao lado dos
        solos de Synyster.
      - Autodidata, aprendeu a tocar sozinho antes de fundar a banda com M.
        Shadows.
    setup:
      - tipo: Guitarra
        original:
          nome: Schecter Zacky Vengeance ZV-H6LLYW66D
          preco: ~R$ 9.000
        low:
          nome: Schecter Omen Extreme-6
          preco: ~R$ 3.200
          obs: Corpo parecido com humbuckers quentes pro riff pesado.
      - tipo: Amplificação
        original:
          nome: Jet City 100HDM
          preco: ~R$ 8.500
        low:
          nome: Boss Katana 100 MkII
          preco: ~R$ 2.400
          obs: Ganho encorpado à meia potência, como Zacky usa ao vivo.
  - nome: Johnny Christ
    funcao: Baixo
    status: Baixista
    anos: 2002 – hoje
    inspiracoes: Robert Trujillo, Cliff Burton
    resumo: Johnny Christ (Jonathan Seward) assumiu o baixo do Avenged
      Sevenfold em 2002, aos 18 anos, depois que a banda passou por uma
      sucessão rápida de baixistas nos primeiros anos (Matt Wendt, Justin
      Sane e Dameon Ash). Desde então é o responsável pela base grave que
      sustenta o confronto entre as duas guitarras da banda, com uma pegada
      agressiva e presente. Tem baixo signature pela Schecter.
    pontos:
      - Entrou pra banda em 2002, ainda adolescente, depois de uma sucessão
        rápida de baixistas.
      - Levada agressiva e presente que reforça o peso das duas guitarras.
    setup:
      - tipo: Baixo
        original:
          nome: Schecter Johnny Christ Signature
          preco: ~R$ 7.500
        low:
          nome: Squier Classic Vibe Jazz Bass
          preco: ~R$ 3.200
          obs: Base pro tom agressivo com captações ativas por menos.
      - tipo: Amplificação
        original:
          nome: Gallien-Krueger 2001RB
          preco: ~R$ 12.000
        low:
          nome: Ampeg BA-115
          preco: ~R$ 3.500
          obs: Combo compacto com grave definido.
  - nome: Brooks Wackerman
    funcao: Bateria
    status: Baterista
    anos: 2015 – hoje
    inspiracoes: Punk rock, hardcore, Bad Religion
    resumo: Brooks Wackerman é o baterista do Avenged Sevenfold desde 2015,
      quando assumiu o posto após a saída de Arin Ilejay — que havia sucedido
      brevemente Mike Portnoy nas turnês e gravações seguintes à morte do
      baterista fundador, Jimmy "The Rev" Sullivan, em 2009. Antes de entrar
      pro A7X, Wackerman já tinha um currículo extenso, com passagens pela
      banda de punk rock Bad Religion e pela banda de Jack Black, Tenacious
      D. Seu primeiro álbum de estúdio com o Avenged Sevenfold foi "The
      Stage" (2016), e ele segue como baterista da banda até o momento, sem
      indícios de mudança na formação.
    pontos:
      - Levada técnica com groove herdado do punk rock e do hardcore.
      - Precisão em andamentos rápidos e viradas complexas ao vivo.
    setup:
      - tipo: Bateria
        original:
          nome: DW Collector's Series (Santa Monica maple)
          preco: ~R$ 40.000
        low:
          nome: Pearl Export EXX
          preco: ~R$ 6.500
          obs: Kit versátil com boa projeção pra ensaio e palco.
      - tipo: Pratos
        original:
          nome: Zildjian A Custom / K Custom
          preco: ~R$ 10.000
        low:
          nome: Zildjian ZBT
          preco: ~R$ 2.800
          obs: Entrada na linha Zildjian com bom brilho.
albuns:
  - nome: City of Evil
    ano: 2005
    capa: /images/albuns/avenged-sevenfold-city-of-evil.jpg
    capaAlt: Capa do álbum City of Evil, de Avenged Sevenfold
    descricao: Terceiro álbum e o divisor de águas que afastou a banda do
      metalcore dos dois primeiros discos rumo a um heavy metal mais
      clássico, sem vocais guturais e com forte influência da NWOBHM.
    temas:
      - tema: Mal bíblico e corrupção institucionalizada
        explicacao: A faixa "Beast and the Harlot" usa a imagem apocalíptica
          da Besta de sete cabeças e da mulher sentada sobre ela, do livro
          bíblico de Apocalipse, pra descrever uma sociedade tomada pela
          corrupção — o próprio título do álbum vem de um verso dessa
          música.
      - tema: Vício, excesso e a inspiração de Hunter S. Thompson
        explicacao: A faixa "Bat Country" nasceu do livro "Medo e Delírio em Las
          Vegas", de Hunter S. Thompson — o título vem do trecho em que o
          protagonista, em surto alucinógeno, grita que aquilo ali é "terra
          de morcegos" durante uma viagem de carro a Las Vegas. A letra foi
          fechada pouco antes do suicídio do próprio Thompson, em fevereiro
          de 2005, transformando a faixa numa homenagem não planejada ao
          escritor.
      - tema: Efêmero, perda e o "aproveite o dia"
        explicacao: A faixa "Seize the Day" trata da urgência de viver plenamente
          diante da certeza da perda, num tom mais introspectivo que
          contrasta com a agressividade do restante do disco.
      - tema: Alienação e decadência urbana
        explicacao: A faixa "Trashed and Scattered" descreve a sensação de estar
          preso numa rotina sufocante numa metrópole hostil, enquanto
          "M.I.A." lida com o sentimento de estar ausente ou perdido mesmo
          fisicamente presente.
      - tema: Uma cidade dourada e corrompida
        explicacao: A faixa-título fecha o disco descrevendo uma cidade
          brilhante construída em ouro, bem distante da inocência — um
          retrato apocalíptico que amarra o conceito solto do álbum em
          torno da corrupção e do pecado.
    scoresCriticos:
      - fonte: AllMusic
        nota: 3.5
        notaMax: 5
      - fonte: Rolling Stone
        nota: 3
        notaMax: 5
      - fonte: Metal Hammer
        nota: 8
        notaMax: 10
      - fonte: Sonar Musical
        nota: 8.7
        notaMax: 10
    musicasPopulares:
      - nome: Bat Country
        streams: 338 milhões
      - nome: Seize the Day
        streams: 193 milhões
      - nome: Beast and the Harlot
        streams: 131 milhões
      - nome: M.I.A.
        streams: 74 milhões
      - nome: Trashed and Scattered
        streams: 41 milhões
    secoesAnalise:
      - titulo: Gravação e produção
        texto: Gravado no início de 2005 e lançado em 6 de junho daquele
          ano pela Warner Bros./Hopeless Records, City of Evil marcou a
          virada sonora da banda rumo a um heavy metal mais tradicional,
          com forte pegada da NWOBHM — a mixagem ficou a cargo de Andy
          Wallace. M. Shadows passou meses treinando com um vocal coach que
          já havia trabalhado com Axl Rose e Chris Cornell, pra abandonar
          de vez os vocais guturais dos dois primeiros discos.
      - titulo: Impacto e legado
        texto: A faixa "Bat Country" se tornou um dos maiores sucessos da carreira
          da banda, chegando ao topo do Total Request Live da MTV e às
          primeiras posições das paradas de rock americanas. O álbum foi
          certificado Platina pela RIAA em 2009 e já ultrapassou 2,5
          milhões de cópias vendidas no mundo, consolidando o Avenged
          Sevenfold como um dos nomes centrais do heavy metal americano dos
          anos 2000.
      - titulo: Curiosidade
        texto: City of Evil foi o primeiro disco da banda sem nenhum vocal
          gutural do estilo metalcore que marcava "Sounding the Seventh
          Trumpet" (2001) e "Waking the Fallen" (2003) — uma mudança que
          dividiu parte da base de fãs mais antiga, mas abriu caminho pro
          sucesso comercial que viria a seguir.
  - nome: Avenged Sevenfold
    ano: 2007
    capa: /images/albuns/avenged-sevenfold-avenged-sevenfold.jpg
    capaAlt: Capa do álbum Avenged Sevenfold, de Avenged Sevenfold
    descricao: Quarto álbum, autoproduzido pela primeira vez e também
      conhecido como "the White Album" por causa da capa minimalista — um
      som mais aberto e melódico, e o último disco gravado com o baterista
      fundador Jimmy "The Rev" Sullivan.
    temas:
      - tema: Saudade e distância durante a turnê
        explicacao: A faixa "Dear God" nasceu de uma brincadeira no estúdio, em Big
          Bear, quando Synyster Gates e The Rev especulavam sobre como seria
          fácil compor um hit country-pop — a faixa que resultou disso virou
          uma carta sincera de M. Shadows sobre a distância da esposa
          enquanto a banda estava em turnê.
      - tema: Negação e autodestruição num relacionamento
        explicacao: A faixa "Almost Easy" retrata uma mente que já sabe que causou um
          estrago irreparável mas ainda tenta se convencer do contrário,
          alternando entre culpa e súplica.
      - tema: Horror gótico e vingança do além
        explicacao: A faixa "A Little Piece of Heaven" narra, em tom quase teatral e
          com arranjos orquestrais escritos pelo próprio The Rev, a história
          de um homem que mata a namorada por medo de ser rejeitado — e
          depois é assombrado e morto por ela, ressuscitada.
      - tema: Arrependimentos e o que fica pra trás
        explicacao: A faixa "Afterlife" usa a metáfora de cair na vida após a morte e
          discutir com os deuses pra falar sobre a urgência de resolver
          pendências emocionais, como fazer as pazes com o pai ou dizer que
          se ama quem se ama, antes que seja tarde.
      - tema: Crítica a quem julga soldados à distância
        explicacao: A faixa "Critical Acclaim" foi escrita depois que M. Shadows viu
          amigos próximos servirem no Iraque, e ataca quem critica os
          soldados sem entender o peso das decisões tomadas em combate.
    scoresCriticos:
      - fonte: Metacritic
        nota: 56
        notaMax: 100
      - fonte: AllMusic
        nota: 3
        notaMax: 5
      - fonte: Sonar Musical
        nota: 7.5
        notaMax: 10
    musicasPopulares:
      - nome: Afterlife
        streams: 400 milhões
      - nome: A Little Piece of Heaven
        streams: 380 milhões
      - nome: Dear God
        streams: 292 milhões
      - nome: Almost Easy
        streams: 191 milhões
      - nome: Critical Acclaim
        streams: 124 milhões
    secoesAnalise:
      - titulo: Gravação e produção
        texto: Gravado em 2007 nos estúdios Capitol, Eldorado e Sunset
          Sound, em Los Angeles, foi o primeiro álbum autoproduzido pela
          banda, sem um produtor externo no comando. O som resultante é
          mais aberto e melódico que City of Evil, com riffs de andamento
          médio e mais espaço nos arranjos.
      - titulo: O último disco com The Rev
        texto: Este foi o último álbum de estúdio gravado com o baterista
          fundador Jimmy "The Rev" Sullivan, que morreria dois anos depois,
          durante a produção do disco seguinte. Sullivan também compôs os
          arranjos orquestrais de "A Little Piece of Heaven", mostrando uma
          versatilidade que ia muito além da bateria.
      - titulo: Recepção dividida
        texto: A crítica ficou dividida — o Metacritic registrou uma média
          de 56/100, entre elogios ao ineditismo sonoro e críticas por
          falta de coesão entre os estilos das faixas. Ainda assim, o disco
          foi um sucesso comercial, estreando na 4ª posição da Billboard
          200.
  - nome: Nightmare
    ano: 2010
    capa: /images/albuns/avenged-sevenfold-nightmare.jpg
    capaAlt: Capa do álbum Nightmare, de Avenged Sevenfold
    descricao: Quinto álbum, gravado após a morte do baterista fundador
      Jimmy "The Rev" Sullivan, com Mike Portnoy (Dream Theater) nas
      baquetas — um disco de luto que também guarda as últimas composições e
      vocais do próprio Rev.
    temas:
      - tema: Punição e as consequências dos próprios atos
        explicacao: A faixa-título é narrada pela perspectiva de alguém já
          condenado ao inferno, que zomba de quem cometeu os mesmos erros —
          segundo M. Shadows, foi a única letra do disco que não precisou
          ser adaptada após a morte de Sullivan, porque ele amava tanto o
          texto original que a banda preferiu manter intacto.
      - tema: Luto e homenagem a um amigo perdido
        explicacao: A faixa "So Far Away" é a homenagem mais direta a The Rev,
          lidando com a dor da ausência e a dificuldade de aceitar a perda
          de alguém tão próximo.
      - tema: Invasão da privacidade em meio ao luto
        explicacao: A faixa "Natural Born Killer" reflete, entre outras leituras, o
          desconforto da banda com a cobertura da mídia sobre a morte de
          Sullivan, pedindo espaço pra lidar com a perda longe dos
          holofotes.
      - tema: Colapso emocional e sensação de aprisionamento
        explicacao: A faixa "Buried Alive" usa a imagem de ser enterrado vivo pra
          descrever um estado de paralisia emocional — a música já estava
          pronta antes da morte de The Rev, que chegou a gravar uma demo
          cantando a letra original.
      - tema: Últimas palavras e aceitação da própria morte
        explicacao: A faixa "Fiction" foi a última música que The Rev escreveu,
          entregue à banda no Natal de 2009 sob o título provisório
          "Death" — a faixa final do disco carrega seus próprios vocais,
          gravados como uma espécie de despedida.
    scoresCriticos:
      - fonte: Metacritic
        nota: 62
        notaMax: 100
      - fonte: 411Mania
        nota: 9
        notaMax: 10
      - fonte: Sonar Musical
        nota: 8.3
        notaMax: 10
    musicasPopulares:
      - nome: Nightmare
        streams: 514 milhões
      - nome: So Far Away
        streams: 318 milhões
      - nome: Buried Alive
        streams: 192 milhões
      - nome: Natural Born Killer
        streams: 89 milhões
      - nome: Fiction
        streams: 43 milhões
    secoesAnalise:
      - titulo: A morte de The Rev e a decisão de continuar
        texto: Em 28 de dezembro de 2009, pouco depois de a banda terminar
          as demos do álbum, o baterista Jimmy "The Rev" Sullivan foi
          encontrado morto em casa, aos 28 anos — a autópsia posterior
          apontou overdose acidental por uma combinação de álcool e
          remédios controlados. O grupo chegou a cogitar o fim da banda,
          mas decidiu seguir em frente depois do incentivo dos próprios
          pais de Sullivan, que sabiam o quanto ele queria ver o disco
          terminado.
      - titulo: Gravação e produção
        texto: Produzido por Mike Elizondo e mixado por Andy Wallace, o
          álbum teve as baquetas assumidas por Mike Portnoy, então
          baterista do Dream Theater, que também acompanhou a banda nas
          turnês de divulgação até o fim de 2010. Ainda assim, boa parte da
          composição rítmica e das ideias originais partiu do próprio
          Sullivan, creditado em várias faixas.
      - titulo: Recepção e sucesso comercial
        texto: Nightmare estreou na 1ª posição da Billboard 200 — a
          primeira vez que o Avenged Sevenfold chegava ao topo da parada
          americana — e recebeu certificação de Platina pela RIAA, além de
          Ouro no Reino Unido e na Nova Zelândia.
  - nome: Hail to the King
    ano: 2013
    capa: /images/albuns/avenged-sevenfold-hail-to-the-king.jpg
    capaAlt: Capa do álbum Hail to the King, de Avenged Sevenfold
    descricao: Sexto álbum, com um som propositalmente mais direto e
      "clássico", inspirado abertamente em Black Sabbath, Iron Maiden,
      Metallica e Guns N' Roses — o disco que consolidou a banda como
      headliner de festivais.
    temas:
      - tema: A sedução do mal como figura protetora
        explicacao: A faixa "Shepherd of Fire", de abertura do álbum, é cantada da
          perspectiva do próprio demônio, que se apresenta como um guia
          simpático às dores humanas enquanto oferece promessas enganosas —
          a banda construiu a introdução da faixa como se o ouvinte
          estivesse literalmente entrando no inferno, com sinos e fogo na
          ambientação sonora.
      - tema: Obediência cega a reis e líderes
        explicacao: A faixa-título narra, na superfície, a história de um
          rei tirano que extermina quem não jura lealdade a ele — mas M.
          Shadows explicou que a letra é, na verdade, uma pergunta sobre
          por que a sociedade sempre precisa seguir reis, presidentes e
          papas em vez de pensar por conta própria.
      - tema: A guerra dentro da própria cabeça
        explicacao: A faixa "This Means War" não fala de conflito militar, mas do
          embate interno entre viver com as próprias decisões e conviver
          com os próprios defeitos, segundo M. Shadows.
      - tema: Um requiem ao contrário
        explicacao: A faixa "Requiem" inverte a lógica das preces sacras
          tradicionais — em vez de pedir salvação a um deus bondoso, a
          letra invoca o senhor das trevas pra ressuscitar o inferno,
          misturando súplica e desejo de destruição em versos cantados em
          latim.
    scoresCriticos:
      - fonte: Metacritic
        nota: 70
        notaMax: 100
      - fonte: Kerrang!
        nota: 80
        notaMax: 100
      - fonte: Sonar Musical
        nota: 8.5
        notaMax: 10
    musicasPopulares:
      - nome: Hail to the King
        streams: 822 milhões
      - nome: Shepherd of Fire
        streams: 303 milhões
      - nome: This Means War
        streams: 148 milhões
      - nome: Coming Home
        streams: 59 milhões
      - nome: Requiem
        streams: 35 milhões
    secoesAnalise:
      - titulo: Gravação e produção
        texto: Produzido novamente por Mike Elizondo, Hail to the King foi
          uma escolha deliberada de soar menos técnico e mais "estádio" do
          que os discos anteriores, com riffs mid-tempo e refrões diretos.
          A banda foi tão aberta sobre suas referências que, anos depois, M.
          Shadows admitiu, em entrevista à revista Revolver, que o grupo
          "cortou perto demais" o som do Black Album do Metallica em
          algumas faixas.
      - titulo: Recepção crítica dividida
        texto: O disco recebeu uma resposta mista da crítica — o Metacritic
          registrou média de 70/100, entre quem elogiou o resgate de uma
          sonoridade mais clássica e crua (o jornal The Guardian chamou o
          resultado de "tão vital quanto as bandas que o inspiraram") e
          quem via na mudança uma perda de identidade da banda.
      - titulo: Legado comercial
        texto: Hail to the King estreou na 1ª posição da Billboard 200,
          tornando-se o segundo álbum consecutivo do Avenged Sevenfold a
          chegar ao topo da parada americana, além de ser o primeiro disco
          da banda a liderar as paradas do Reino Unido.
---
